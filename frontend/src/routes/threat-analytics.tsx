    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {(() => {
          ...
          const score = totalAnomalies > 0 ? Math.min(100, Math.round((criticalCount / Math.max(totalAnomalies, 1)) * 100)) : 0;
          const scoreLabel = score > 70 ? "Critical" : score > 40 ? "Elevated" : score > 15 ? "Moderate" : "Low";

          const cards = [
            { label: "Threat Score", value: `${score} / 100`, sub: scoreLabel, icon: Shield, tone: "warning" as const },
            { label: "Techniques Seen", value: String(techniquesCount), sub: `from ${totalAnomalies} anomalies`, icon: Target, tone: "default" as const },
            { label: "Critical Threats", value: String(criticalCount), sub: `of ${totalThreats} total`, icon: Zap, tone: "critical" as const },
            { label: "Total Anomalies", value: String(totalAnomalies), sub: `${metrics.blocked || 0} blocked`, icon: TrendingUp, tone: "success" as const },
          ];
